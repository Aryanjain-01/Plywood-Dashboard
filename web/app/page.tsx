...
import { CategoryPieChart, MonthlyRevenueChart, WeeklyTrendChart, PaymentBreakdownChart } from "@/components/dashboard/charts"
...
function StatCard({ label, value, icon: Icon, trend, trendLabel, onClick }: {
    ...
    <div className="flex items-center justify-between border-t border-white/[0.06] px-5 py-3">
      ...
      <div className="flex items-center gap-1">
        ...
        {Array.from({ length: Math.min(pages, 7) }, (_, i) => {
          ...
          if (n === -1) return <span key="ellipsis" className="px-1 text-xs text-slate-600">…</span>
          return (
            <button
              key={n}
              onClick={() => onChange(n)}
...
function ReceiptModal({ sale, onClose }: { sale: Sale; onClose: () => void }) {
  ...
  function handlePrint() {
    ...
    if (!w) return
    w.document.write(`
      <html><head><title>Receipt ${sale.sale_id}</title>
      <style>
        body { font-family: monospace; font-size: 13px; color: #111; padding: 24px; max-width: 360px; margin: auto; }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-sm rounded-xl border border-white/[0.10] bg-[#161b24] shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div ref={ref} className="px-5 py-4 font-mono text-xs text-slate-300 [&_h2]:text-center [&_h2]:text-sm [&_h2]:font-bold [&_h2]:text-slate-100">
          <h2>PlywoodPro</h2>
          <p className="mb-4 text-center text-slate-500">{formatDate(sale.date)}</p>
          <div className="space-y-1.5">
            <div className="flex justify-between"><span className="text-slate-500">Receipt No</span><span>{sale.sale_id}</span></div>
            <div className="flex justify-between"><span className="text-slate-500">Customer</span><span>{sale.customer}</span></div>
            <div className="flex justify-between"><span className="text-slate-500">Payment</span><span>{sale.payment}</span></div>
            {sale.project !== "General" && <div className="flex justify-between"><span className="text-slate-500">Project</span><span>{sale.project}</span></div>}
          </div>
          <div className="my-3 border-t border-dashed border-white/[0.10]" />
          <div className="space-y-1">
            <div className="flex justify-between text-slate-300">
              <span className="flex-1">{sale.product_name}</span>
            </div>
...
// ─── inline edit row for sales ────────────────────────────────────────────────

function EditSaleRow({ sale, products, onSave, onCancel }: {
  sale: Sale; products: Product[]; onSave: (s: Sale) => void; onCancel: () => void
}) {
  ...
  function save() {
    const total = qty * unitPrice * (1 - discount / 100)
    onSave({ ...sale, qty, unit_price: unitPrice, discount_pct: discount, total_amount: Number(total.toFixed(2)), customer, payment })
  }

  return (
    <tr className="border-b border-indigo-500/20 bg-indigo-500/[0.04]">
      <td className="px-5 py-2 text-slate-500 text-xs">{formatDate(sale.date)}</td>
      ...
      <td className="px-5 py-2">
        <Badge colorClass={categoryColors[sale.category]}>{sale.category}</Badge>
      </td>
      <td className="px-5 py-2">
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (product) setSaleUnitPrice(product.base_price)
  }, [saleProductId, products])

  // reset to page 1 when filters change
  useEffect(() => { setSalesPage(1) }, [salesSearch, salesCatFilter, salesPayFilter, salesDateFrom, salesDateTo, salesSortCol, salesSortDir])
  ...
  // recent activity (last 8 sales desc)
  ...
  // analytics: weekly trend (last 12 weeks)
  ...
  const topCustomers = useMemo(() => {
    ...
    for (const s of sales) {
      const cur = m.get(s.customer) || { revenue: 0, count: 0 }
      m.set(s.customer, { revenue: cur.revenue + Number(s.total_amount || 0), count: cur.count + 1 })
    }
  ...
  const filteredSales = useMemo(() => {
    ...
    return result
  }, [sales, salesSearch, salesCatFilter, salesPayFilter, salesDateFrom, salesDateTo, salesSortCol, salesSortDir])
  ...
  function exportCSV(rows: Sale[]) {
    const headers = ["Sale ID", "Date", "Product", "Category", "Qty", "Unit Price", "Discount %", "Total", "Customer", "Project", "Payment", "Note"]
    const lines = [headers.join(","), ...rows.map((s) =>
      [s.sale_id, s.date, `"${s.product_name}"`, s.category, s.qty, s.unit_price, s.discount_pct, s.total_amount, `"${s.customer}"`, `"${s.project}"`, s.payment, `"${s.note}"`].join(",")
    )]
  ...
  async function addSale(formData: FormData) {
    ...
    const payload: Sale = {
      ...
      qty, unit_price: unitPrice, discount_pct: discount,
  ...
  async function addProduct(formData: FormData) {
    ...
    if (products.find((p) => p.id === payload.id)) {
      showToast("Product ID already exists", "err"); return
    }
    setSubmitLoading(true)
  ...
  const hasDateFilter = salesDateFrom || salesDateTo
  const hasAnyFilter = salesSearch || salesCatFilter || salesPayFilter || hasDateFilter

  return (
    <div className="flex min-h-screen bg-[#0d1117]">
      <Sidebar page={page} setPage={setPage} salesCount={sales.length} />
        ...
        <div className="mx-auto max-w-6xl animate-fade-in">
          ...
          {!loading && page === "dashboard" && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
                ...
                <StatCard label="Units Sold" value={totalUnits.toLocaleString("en-IN")} icon={ShoppingCart} trend={trends.unitsTrend} trendLabel="vs last 30d" />
          ...
          {!loading && page === "sales" && (
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <div className="w-52">
                  <SearchBar value={salesSearch} onChange={setSalesSearch} placeholder="Search product, customer…" />
                </div>
                ...
                {hasAnyFilter && (
                  <button onClick={() => { setSalesSearch(""); setSalesCatFilter(""); setSalesPayFilter(""); setSalesDateFrom(""); setSalesDateTo("") }}
                        ...
                        <thead>
                          <tr className="border-b border-white/[0.07]">
                            {[
                              ...
                              { key: null, label: "Category" }, { key: null, label: "Payment" },
                        ...
                        <tbody>
                          {pagedSales.map((s) =>
                            editingSaleId === s.sale_id ? (
                              <EditSaleRow key={s.sale_id} sale={s} products={products} onSave={saveSaleEdit} onCancel={() => setEditingSaleId(null)} />
                            ) : (
                              <tr key={s.sale_id} className="group border-b border-white/[0.04] transition-colors hover:bg-white/[0.02]">
                                ...
                                <td className="px-5 py-3"><Badge colorClass={categoryColors[s.category]}>{s.category}</Badge></td>
                                <td className="px-5 py-3"><Badge colorClass={paymentColors[s.payment] ?? "bg-slate-500/15 text-slate-300"}>{s.payment}</Badge></td>
          ...
          {!loading && page === "products" && (
            <div className="space-y-4">
              <PageHeader
                title="Products"
                description={`${filteredProducts.length} of ${products.length} items`}
                action={
              ...
              <Card>
                {filteredProducts.length === 0 ? (
                  <EmptyState message={products.length === 0 ? "No products yet. Add one in Settings." : "No results."} />
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-white/[0.07]">
                          {[
                            ...
                            { key: null, label: "Size" }, { key: null, label: "Thickness" }, { key: null, label: "Unit" },
                            { key: "price", label: "Base Price" }, { key: null, label: "" },
                          ].map(({ key, label }, i) => (
                            <th key={label + i} onClick={() => key && toggleSort(key, productsSortCol, productsSortDir, setProductsSortCol, setProductsSortDir)}
                              className={`px-5 py-3.5 text-xs font-medium text-slate-500 ${i === 6 ? "text-right" : "text-left"} ${key ? "cursor-pointer select-none hover:text-slate-300" : ""}`}>
                      ...
                      <tbody>
                        {filteredProducts.map((p) => (
                          <tr key={p.id} className="group border-b border-white/[0.04] transition-colors hover:bg-white/[0.02]">
                            ...
                            <td className="px-5 py-3 text-slate-400">{p.thickness}</td>
                    ...
                    <form action={addSale} className="space-y-4">
                      <Field label="Product">
                        <Select name="product_id" required value={saleProductId}
                          ...
                          {products.map((p) => <option key={p.id} value={p.id}>{p.name} — {formatINR(p.base_price)}/{p.unit}</option>)}
                        </Select>
                      </Field>
            ...
            <div className="space-y-5">
              ...
              <Card>
                ...
                <CardContent>
                  {topCustomers.length === 0 ? (
                    <p className="text-sm text-slate-600">No customer data yet.</p>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-white/[0.07]">
                            <th className="py-2.5 text-left text-xs font-medium text-slate-500">#</th>
                            <th className="py-2.5 text-left text-xs font-medium text-slate-500">Customer</th>
                            <th className="py-2.5 text-right text-xs font-medium text-slate-500">Orders</th>
                            <th className="py-2.5 text-right text-xs font-medium text-slate-500">Total Revenue</th>
                            <th className="py-2.5 text-right text-xs font-medium text-slate-500">Avg Order</th>
                          </tr>
                        </thead>
                        <tbody>
              ...
              <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                ...
                <Card>
                  ...
                  <CardContent>
                    {paymentData.length === 0 ? <p className="text-sm text-slate-600">No data.</p> : (
                      <div className="space-y-2.5">
                        {paymentData.map(({ name, value }) => {
                          ...
                          return (
                            <div key={name}>
                              <div className="mb-1 flex items-center justify-between">
                                <Badge colorClass={paymentColors[name]}>{name}</Badge>
                                <div className="flex items-center gap-3">
                                  <span className="text-xs text-slate-500">{pct.toFixed(1)}%</span>
                                  <span className="w-24 text-right text-sm font-medium tabular-nums text-slate-300">{formatINR(value)}</span>
                                </div>
          ...
          {!loading && page === "settings" && (
            <div className="space-y-5">
              <PageHeader title="Settings" description="Manage products and business info" />
                ...
                <div className="space-y-4">
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-2">
                      <p className="text-xs text-slate-600">Sales and products are stored in <code className="rounded bg-white/[0.06] px-1 text-slate-400">data/</code> as JSON files.</p>
